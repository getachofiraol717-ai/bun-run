-- Disable the faulty free-text message limiter while keeping usage audit rows.
-- This protects AI Tutor even if an older edge function deployment still calls the RPC.
...
DECLARE
  ...
  _effective_limit int := CASE
    WHEN _kind = 'message' THEN 2147483647
...
BEGIN
  ...
  IF _kind = 'message' THEN
    INSERT INTO public.ai_usage (user_id, kind, provider) VALUES (_uid, _kind, _provider);
    RETURN jsonb_build_object('allowed', true, 'used', 0, 'limit', _effective_limit, 'rate_limit_disabled', true);
  END IF;
